...
if nargin < 6
  ...
  if ~linearModel.PlasmaCurrentInfo.Kplasm 
    ...
    if isempty(find(strcmp(inputNames,'Vpl'),1))
      ...
      % indexVDin = [indexVD, linearModel.nOfInputs];
...
elseif nargin == 6
  ...
  if isempty(find(strcmp(cdNames,'Ipl'),1)) == 0
  ...
  if isempty(find(strcmp(vdNames,'Vpl'),1)) == 0
    indexVD = [indexVD, linearModel.PlasmaCurrentInfo.StatePosition];
  end
  
  % state indexes of the eddy current
  indexEddy = linearModel.EddyCurrentsInfo.StatePosition;
  %   if (isempty(find(strcmp(vdNames,'Vpl'),1)) == 1) && (isempty(find(strcmp(cdNames,'Ipl'))) == 1)
  %     indexEddy = [indexEddy, linearModel.PlasmaCurrentInfo.StatePosition];
  %   end
  
  if ~linearModel.PlasmaCurrentInfo.Kplasm 
    % Consider plasma as a passive circuit
    if isempty(find(strcmp(inputNames,'Vpl'),1)) && isempty(find(strcmp(cdNames,'Ipl'),1))
      indexEddy = [indexEddy, linearModel.PlasmaCurrentInfo.StatePosition];
